...
Eps = 1e-5 # regularization strength works fine from 1e-5 to 1e-3. Lower is better but slower
# get elastic net path
_ , coefs, _ = enet_path(x_std, y_std, l1_ratio=0.1, eps=Eps, n_alphas=100, alphas=None,
...
features_index = library1.BackwardElimination(X, Y, FeaturesAll, FeaturesReduced, \
    Method='sequential', Criterion='MSE', N_of_features_to_select=10, idx=idx, \
    PlotPath=True, StorePath=True, FileName='B1', N_of_last_iterations_to_store=20, verbose=True)
idx_corr = library1.ClassifyCorrelatedFeatures(X, features_index, MinCorrelation=0.95, Model=1, verbose=True)
active_features = library1.FindAlternativeFit(x_std, y_std, features_index, idx_corr, Fit=1, Method='MSE', verbose=True)
writeResults = pd.ExcelWriter('FileName.xlsx', engine='openpyxl')
library1.Results_to_xls(writeResults, '1', active_features, X, Y, FeaturesAll, FeaturesReduced)
writeResults.save()
