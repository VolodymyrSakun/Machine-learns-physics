...
def least_sq(X, Y, zero):
    coef = pinv(X, rcond=zero).dot(Y)
    return coef
...
def set_split(X, Y, test_size=0.1, Shuffle=True):
# abalog of train_test_split
    if test_size >= 1 or test_size <= 0:
        print('split parameter is chosen incorectly. Must be between 0 and 1')
        return
# check Y has correct shape
    ...
    if X.shape[0] != Y.shape[0]:
        print('Number of observations in X is not equal number of responses')
        return
# size of training set    
    split_point = int(Y.shape[0] * (1 - test_size))
    if Shuffle:
        data = np.concatenate((X, Y), axis=1)
        shuffle(data, random_state=101)
        Y[:, 0] = data[:, -1]
        X = np.delete(data, -1, 1)
    X_train = np.zeros(shape=(split_point, X.shape[1]), dtype=float)
    X_test = np.zeros(shape=(X.shape[0] - split_point, X.shape[1]), dtype=float)
    Y_train = np.zeros(shape=(split_point, 1), dtype=float)
    Y_test = np.zeros(shape=(X.shape[0] - split_point, 1), dtype=float)
    for i in range(0, split_point, 1):
        Y_train[i, 0] = Y[i, 0]
        X_train[i, :] = X[i, :]
    for i in range(0, X.shape[0] - split_point, 1):
        Y_test[i, 0] = Y[i + split_point, 0]
        X_test[i, :] = X[i + split_point, :]
...
# do not forget to check for zeros in coef later    
    Results = pd.DataFrame(np.zeros(shape = (NofSelectedFeatures, 16)).astype(float), \
    ...
    for i in range(0, NofSelectedFeatures, 1):
        ...
        Results.loc[i]['Bond 1'] = FeaturesReduced[index].distances[0].Atom1.Symbol + '-' + FeaturesReduced[index].distances[0].Atom2.Symbol
        Results.loc[i]['Power 1'] = FeaturesReduced[index].powers[0]
        ...
        if FeaturesReduced[index].nDistances >= 2:
            Results.loc[i]['Bond 2'] = FeaturesReduced[index].distances[1].Atom1.Symbol + '-' + FeaturesReduced[index].distances[1].Atom2.Symbol
            Results.loc[i]['Power 2'] = FeaturesReduced[index].powers[1]
        ...
        if FeaturesReduced[index].nDistances == 3:
            ...
            Results.loc[i]['Bond 3'] = FeaturesReduced[index].distances[2].Atom1.Symbol + '-' + FeaturesReduced[index].distances[2].Atom2.Symbol
            Results.loc[i]['Power 3'] = FeaturesReduced[index].powers[2]
            ...
            Results.loc[i]['Bond 3'] = ''
            Results.loc[i]['Power 3'] = ''
            Results.loc[i]['Intermolecular 3'] = ''
        counter = 0
...
# fit raw data
        ...
        while count < N_of_last_iterations_to_store:
            ...
            for i in features_index: # index of full set
                ...
                Table.loc[j]['Bond 1'] = FeaturesReduced[i].distances[0].Atom1.Symbol + '-' + FeaturesReduced[i].distances[0].Atom2.Symbol
                Table.loc[j]['Power 1'] = FeaturesReduced[i].powers[0]
                ...
                if FeaturesReduced[i].nDistances >= 2:
                    Table.loc[j]['Bond 2'] = FeaturesReduced[i].distances[1].Atom1.Symbol + '-' + FeaturesReduced[i].distances[1].Atom2.Symbol
                    Table.loc[j]['Power 2'] = FeaturesReduced[i].powers[1]
                    if max_distances_in_feature < 2:
                        max_distances_in_feature = 2
                ...
                if FeaturesReduced[i].nDistances == 3:
                    if max_distances_in_feature < 3:
                        max_distances_in_feature = 3
                    Table.loc[j]['Bond 3'] = FeaturesReduced[i].distances[2].Atom1.Symbol + '-' + FeaturesReduced[i].distances[2].Atom2.Symbol
                    Table.loc[j]['Power 3'] = FeaturesReduced[i].powers[2]
                    if FeaturesReduced[i].distances[2].isIntermolecular != 0: # 1 = Yes 0 = No
                        Table.loc[j]['Intermolecular 3'] = 'Yes'
                    else:
                        Table.loc[j]['Intermolecular 3'] = 'No'
                else:
                    Table.loc[j]['Bond 3'] = ''
                    Table.loc[j]['Power 3'] = ''
                    Table.loc[j]['Intermolecular 3'] = ''                    
                counter = 0
