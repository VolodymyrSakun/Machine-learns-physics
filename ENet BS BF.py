...
if __name__ == '__main__':
    ...
    if FinalAdjustment == 'Backward sequential and Search Alternative':
        ...
        while len(idx) > 2:
            FeatureSize = len(idx)
            if FeatureSize > LastIterationsToStore:
                print(FeatureSize)
            ...
            if len(idx_backward) > LastIterationsToStore:
                idx = idx_backward
                continue
#            idx_corr = library2.ClassifyCorrelatedFeatures(x_std, idx_backward, MinCorrelation=MinCorr, Model=1, Corr_Matrix=C, verbose=False)
